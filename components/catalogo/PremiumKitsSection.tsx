...
export function PremiumKitsSection({ lojaSlug, kits, whatsapp }: { lojaSlug: string; kits: Produto[]; preview?: boolean; whatsapp?: string | null }) {
  const kit = kits[0];
  const price = kit ? kit.preco_promocional ?? kit.preco : 149.9;
  const oldPrice = kit?.preco_promocional ? kit.preco : 189.9;
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="relative mx-auto grid max-w-[1400px] gap-12 lg:min-h-[640px] lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-[clamp(56px,7vw,110px)]">
        <div>
          ...
          <motion.div variants={fadeInUp} className="mt-7">
            <a href={whatsappUrl ?? "#"} target={whatsappUrl ? "_blank" : undefined} className="inline-flex h-[58px] items-center gap-3 rounded-full bg-[linear-gradient(135deg,#D7AE4A,#A87921)] px-8 font-bold text-white shadow-[0_18px_40px_rgba(168,121,33,0.22)] transition hover:-translate-y-1 hover:shadow-[0_24px_55px_rgba(168,121,33,0.30)] max-sm:w-full max-sm:justify-center">
        ...
        <motion.div variants={fadeInUp} className="relative flex items-center justify-center">
          ...
          <article className="relative z-10 w-full max-w-[440px] rounded-[32px] border border-[#C9A24D]/20 bg-white/78 p-[18px] shadow-[0_34px_90px_rgba(80,55,25,0.16)]">
            <div className="relative aspect-square overflow-hidden rounded-[24px] bg-[#EADBC8]">
              <Image src={kit?.imagem_url || "https://images.unsplash.com/photo-1512163143273-bde0e3cc7407?q=90&w=1400&auto=format&fit=crop"} alt={kit?.nome || "Kit Essencial Dourado"} fill className="object-cover sepia-[0.08] saturate-105 brightness-105" />
              ...
              <span className="absolute right-5 top-5 grid size-[42px] place-items-center rounded-full border border-[#C9A24D]/20 bg-white/88 text-[#8D6A35]">
            ...
            <div className="px-2 pb-1 pt-6">
              ...
              <div className="mt-4 flex items-baseline gap-2">
                ...
                <span className="text-sm text-[#AAA098] line-through">{formatPrice(oldPrice)}</span>
              </div>
            </div>
          </article>
        </motion.div>
