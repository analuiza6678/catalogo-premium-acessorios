...
export function PremiumProductCard({ lojaSlug, produto, preview, compact }: PremiumProductCardProps) {
  ...
  const badge = isOutOfStock(produto) ? "Esgotado" : produto.badge || (isKit ? "Kit" : produto.destaque ? "Destaque" : null);
  ...
  return (
    <motion.article whileHover={hoverLift} className={`group overflow-hidden rounded-[34px] border border-white/80 bg-white/80 p-3 shadow-[0_24px_75px_rgba(58,42,36,0.10)] backdrop-blur ${isKit ? "bg-[#F8DDEB]/45" : ""}`}>
      <div className={`relative overflow-hidden rounded-[28px] bg-[#FAF3E8] ${compact ? "aspect-[5/4]" : "aspect-[4/5]"}`}>
        ...
        <div className="absolute inset-x-3 bottom-3 translate-y-4 opacity-0 transition duration-300 group-hover:translate-y-0 group-hover:opacity-100">
          <div className="grid gap-2 rounded-[22px] bg-white/86 p-2 shadow-[0_18px_50px_rgba(58,42,36,0.16)] backdrop-blur sm:grid-cols-2">
            {preview ? (
              <Button variant="ghost" className="min-h-10 w-full px-3" onClick={() => toast("Este é um produto de exemplo para visualizar o catálogo.")}>
                <Eye size={16} />
                Ver
              </Button>
            ) : (
            ...
            <Button className="min-h-10 w-full px-3" onClick={addToCart} disabled={isOutOfStock(produto)}>
              <ShoppingBag size={16} />
      ...
      <div className="px-1 pt-4">
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#C9A227]">{produto.categorias?.nome ?? (isKit ? "Kit especial" : "Acessório")}</p>
        <h3 className="mt-2 font-serif text-2xl leading-7 text-[#1E1A18]">{productName(produto.nome)}</h3>
        {description ? <p className="mt-2 line-clamp-2 text-sm leading-6 text-[#76675f]">{description}</p> : null}
        <div className="mt-3 flex items-baseline gap-2">
          <span className="text-xl font-bold text-[#C9A227]">{formatPrice(price)}</span>
          {hasPromo(produto) ? <span className="text-sm text-[#76675f]/45 line-through">{formatPrice(produto.preco)}</span> : null}
        </div>
      </div>
    </motion.article>
  );
